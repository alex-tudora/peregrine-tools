...
export function PdfToJpgTool() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      const rendered = await pdfToImages(buffer, "jpg", quality);
      setProgress(90);
  ...
  }, [file, quality, revokeUrls]);

  const handleDownloadSingle = useCallback(
    (image: RenderedPage) => {
      const baseName = file?.name.replace(/\.pdf$/i, "") ?? "page";
      downloadFile(image.data, `${baseName}-page-${image.pageNumber}.jpg`);
    },
    [file],
  );
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    await downloadAsZip(files, `${baseName}-jpg.zip`);
  }, [images, file]);
