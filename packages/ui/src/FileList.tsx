...
export function FileList({ files, onRemove, onReorder, className = "" }: FileListProps) {
  ...
  return (
    <div className={`rounded-xl border border-slate-200 bg-white divide-y divide-slate-100 ${className}`}>
      {files.map((file, index) => (
        <div
          key={file.id}
          className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-slate-50/50"
        >
          {/* File icon */}
          ...
          {onReorder && files.length > 1 && (
            <div className="flex flex-col gap-0.5">
              <button
                ...
                disabled={index === 0}
                className="flex h-6 w-6 items-center justify-center rounded text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 disabled:opacity-30 disabled:cursor-not-allowed"
